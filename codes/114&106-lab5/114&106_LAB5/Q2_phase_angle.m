...
phase_array_analytical = zeros(length(frequency_array));
index = 1;
...
for driver_angular_freq = start_freq : incremental_freq : end_freq    
    ...
    [t,u] = ode45(@damp_driven_function_for_phase_angle , [start_time : dt : simulationt] , u0); 
    ...
    phase_array_analytical(index) = atan2(1-driver_angular_freq*driver_angular_freq , 2*driver_angular_freq*beta)*(-1) + (pi/2);
    
    index = index + 1;
end

plot(frequency_array , phase_array , 'b' , frequency_array , phase_array_analytical , 'r');
